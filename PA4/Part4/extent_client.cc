...
extent_client::update(extent_protocol::extentid_t eid, const char *name,
    bool is_file, std::string &buf)
{
  extent_protocol::status ret = extent_protocol::OK;
  //as extend client receives string object, so convert 
  //const char * to string
  std::string cname(name);
  ret = cl->call(extent_protocol::update, eid, (int) is_file, name, buf);
  return ret;
}
...
extent_protocol::status
extent_client::unlink(extent_protocol::extentid_t parent, const char *name, std::string &buf)
{
    std::string cname(name);
    extent_protocol::status ret = extent_protocol::OK;
    ret = cl->call(extent_protocol::unlink, parent, cname, buf);
    return ret;
}
