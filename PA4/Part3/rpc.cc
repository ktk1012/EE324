...
// For LAB1: Understand what tihs code is doing:
// What invariant does this function try to maintain for xd_rep_window?
// What does xid_rep_window_front() represent?
...
//	printf("received NEW for client:%d xid:%d xid_rep:%d\n", clt_nonce, xid, rep_xid);
    if (counting)
      updatestat(proc);
    ret = h->fn(args, rep);
    rep1 << xid;
             ...
             marshall &rep)
{
  ...
  // ** YOU FILL THIS IN FOR LAB 1 **
  std::map<unsigned int, std::list<reply_t *> >::iterator iter = reply_window.find(clt_nonce);
  if (iter == reply_window.end()) {
    printf("No reply list correspond to clt nonce\n");
    return;
  }
  std::list<reply_t *>::iterator it = iter->second.begin();
	     ...
	     unsigned int xid, unsigned int xid_rep, marshall &rep)
{
  ...
  std::list<reply_t *>::iterator iter = reply_window[clt_nonce].begin();

  while (iter != reply_window[clt_nonce].end()) {
    if ((*iter)->xid <= xid_rep) {
      delete *iter;
      reply_window[clt_nonce].erase(iter);
      iter = reply_window[clt_nonce].begin();
    }
    ++iter;
  }
  ...
  if(xid <= xid_rep){
    ...
    if (reply) {
      ...
      reply_window[clt_nonce].push_back(new reply_t(xid));
    }
  }
