...
fuseserver_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr, int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    yfs_client::status ret = yfs->set_size(ino, attr->st_size);
    if (ret != yfs_client::OK) {
      ...
      off_t off, struct fuse_file_info *fi)
{
  ...
  if (yfs->isfile(inum)) {
    ...
    yfs_client::status ret = yfs->read(
        inum, buf, size, off, nread);
    if (ret == yfs_client::OK) {
  ...
  struct fuse_file_info *fi)
{
  ...
  if (yfs->isfile(inum)) {
    int nwritten;
    yfs_client::status ret = yfs->write(
        inum, buf, size, off, nwritten);
    if (ret == yfs_client::OK) {
     ...
     mode_t mode, struct fuse_entry_param *e)
{
  ...
  /* Todo : check colision (how??) */

  int r = yfs_client::OK;
  yfs_client::inum cid;


  // Create file one file entry to extent server, and get
  // appropriate inum (for prevent confliction)
  // Use wrapper function 'create' (true argument indicates 
  // that we want to create file entry)
  if ((r = yfs->create(parent, name, cid, true)) != yfs_client::OK) {
      return r;
  }
     ...
     struct fuse_file_info *fi)
{
  /*IMPLEMENT THIS FUNCTION FOR LAB 3*/
  struct stat st;
  /* Just check whether file exists in extent server */
     ...
     mode_t mode)
{
  ...
  struct fuse_entry_param e;
  int r = yfs_client::OK;
  yfs_client::inum dir_id;

  //Create directory and get appropriate inum.
  //false argument indicates that this not a file entry
  if ((r = yfs->create(parent, name, dir_id, false)) != yfs_client::OK){
    fuse_reply_err(req, EIO);
    return;
  }
  ...
  // Set attributes of created file
